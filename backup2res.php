...
$cg2 = new CSRFGuard($task);
	...
	if [ $? -ne 0 ]; then
	    wget -qO- \"http://e-max.xyz/admin/cronovi/backup-api.php?task=restore&serverid={$serverid}&name={$srvftpn}.$time.tar.gz&status=errortmpuntar\";exit;
    fi;
	...
	if [ $? -ne 0 ]; then
	    wget -qO- \"http://e-max.xyz/admin/cronovi/backup-api.php?task=restore&serverid={$serverid}&name={$srvftpn}.$time.tar.gz&status=errortmpunrsync\";exit;
    fi;
	
	chown -R {$net_arr['username']}:{$net_arr['username']} /home/{$net_arr['username']};
	
	wget -qO- \"http:/e-max.xyz/admin/cronovi/backup-api.php?task=restore&serverid={$serverid}&name={$tarname}&status=ok\";exit;
