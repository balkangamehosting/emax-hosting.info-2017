...
<?php
	...
	if [ $? -ne 0 ]; then
	    wget -qO- \"http://e-max.xyz/admin/cronovi/backup-api.php?task=backupapi&serverid={$serverid}&name={$srvftpn}.$time.tar.gz&status=error1\";exit;
    fi;
	...
	if [ $? -ne 0 ]; then
	    wget -qO- \"http://e-max.xyz/admin/cronovi/backup-api.php?task=backupapi&serverid={$serverid}&name={$srvftpn}.$time.tar.gz&status=error2\";exit;
	fi;
	nice -n 19 tar -czf /home/backupnew/tmp/{$srvftpn}.$time.tar.gz *;
	if [ $? -ne 0 ]; then
	    wget -qO- \"http://e-max.xyz/admin/cronovi/backup-api.php?task=backupapi&serverid={$serverid}&name={$srvftpn}.$time.tar.gz&status=error3\";exit;
	fi;
	...
	wget -qO- \"http://e-max.xyz/admin/cronovi/backup-api.php?task=backupapi&serverid={$serverid}&size=\$getsize&name={$srvftpn}.$time.tar.gz&status=ok\";exit;
