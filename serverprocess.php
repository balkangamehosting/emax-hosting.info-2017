...
header("Location: gp-server.php?id=".$serverid);
    ...
    case 'server-boost':
        ...
        query_basic("INSERT INTO `servers` (`id`, `address`, `hostname`, `players`, `maxplayers`, `map`, `status`, `game`, `date_create`, `date_end`, `type`, `rounds`) VALUES (NULL, '".mysql_real_escape_string(trim($_POST['ip']))."', '".mysql_real_escape_string($data['hostname'])."', '".mysql_real_escape_string($data['players'])."', '".mysql_real_escape_string($data['maxplayers'])."', '".mysql_real_escape_string($data['mapname'])."', '".intval($data['status'])."', 'cs16', '".time()."', '{$date_end}', 'cs', '{$row_round['round']}')") or die(mysql_error());
